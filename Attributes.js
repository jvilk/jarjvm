...
function CodeAttribute(javaClassReader, attributeNameIndex) {
	...
	for (i = 0; i < this.codeLength; ) {
		...
		switch(instr) {
			...
			case ByteCode.codes.bipush:
				var int1 = javaClassReader.getIntField(1);
			...
			case ByteCode.codes.new_:
				...
				var index = javaClassReader.getUintField(2);
			...
			case ByteCode.codes.ldc:
				var uint1 = javaClassReader.getUintField(1);
				var constant = CONSTANTPOOL[uint1];
			...
			case ByteCode.codes.lookupswitch:
				...
				//is 4.
				var padding = (4 - ((i+1)%4))%4; //Will be 0-3
				var paddingVal = javaClassReader.getUintField(padding);
				assert(paddingVal == 0);
				var default_ = javaClassReader.getIntField(4);
			...
			case ByteCode.codes.wide:
				...
				{
					var constant = javaClassReader.getIntField(2);
	...
	this.exception = function(exception) {
		for (var exception in this.exceptionTable)
		{
			var exceptionEntry = this.exceptionTable[exception];
...
function ExceptionAttribute(javaClassReader, attributeNameIndex) {
	...
	this.exceptionsIndexTable = new Array();
...
function LineNumberTableAttribute(javaClassReader, attributeNameIndex) {
	...
	this.lineNumberTable = new Array();
...
function LocalVariableTable(javaClassReader, attributeNameIndex) {
	...
	this.localVariableTable = new Array();
